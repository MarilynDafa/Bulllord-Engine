...
public class BLGoogleIAB
{
	...
	{
		void onProductPurchased(String productId, String response, String signature);
	...
	public boolean purchase(Activity activity, String productId)
	{
		...
		try
		{
			...
			if (bundle != null)
			{
				...
				{
					if (syncPurchases() && mEventHandler != null)
						mEventHandler.onProductPurchased(productId, mCurPurchaseData, mCurDataSignature);
					else
	...
	public boolean consumePurchase()
	{
		...
		try
		{
			...
			try
			{
				...
				token = purchase.getString("purchaseToken");
			}
			catch (Exception e)
			{
			...
			{
				int response = mBillingService.consumePurchase(3, mContextPackageName, token);
				if (response == BILLING_RESPONSE_RESULT_OK)
				{
		...
		if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK)
		{
			...
			try
			{
				...
				if (mEventHandler != null)
				{
					...
					mEventHandler.onProductPurchased(productId, purchaseData, dataSignature);				
				}
			}
			catch (Exception e)
			{
