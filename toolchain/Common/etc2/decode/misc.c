...
void detexSetErrorMessage(const char *format, ...) {
	...
	int r = vsnprintf(message, 512, format, args);
	va_end(args);
...
int *nu_levels_out) {
	...
	if (filename_length > 4 && strncmp(filename + filename_length - 4, ".ktx", 4) == 0)
		return detexLoadKTXFileWithMipmaps(filename, max_mipmaps, textures_out, nu_levels_out);
	else if (filename_length > 4 && strncmp(filename + filename_length - 4, ".dds", 4) == 0)
