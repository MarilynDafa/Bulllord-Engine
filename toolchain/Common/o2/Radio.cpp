...
#include "Radio.h"
namespace o2d{
	c_radio::c_radio(c_widget* parent_, const c_widget::s_widget_property& property_)
		:c_widget(WT_RADIO , parent_, property_) , 
	...
	c_radio::~c_radio(){}
	//--------------------------------------------------------
	void c_radio::set_enable(bool enable_)
	{
		if(m_enable != enable_)
		{
			m_enable = enable_;
	...
	//--------------------------------------------------------
	void c_radio::set_pushed(bool pushed_)
	{
		if((m_state == 1) &&!pushed_)
		{
			if((m_parent->get_class() != WT_SCROLLBOX) && ((m_parent->get_class() != WT_TABCTL)))
				_sig_state_change.emit(pushed_);
			m_state = 0;
			c_ui_manager::get_singleton_ptr()->set_dirty(true);
		}
		else if((m_state == 0) &&pushed_)
		{
			_sig_state_change.emit(pushed_);
			m_state = 1;
			c_ui_manager::get_singleton_ptr()->set_dirty(true);
	...
	//--------------------------------------------------------
	void c_radio::set_text(const c_string& text_)
	{
		if(m_text != text_)
		{
			m_text = text_;
	...
	//--------------------------------------------------------
	void c_radio::set_text_color(const c_color& color_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_text_align(e_text_alignment ha_, e_text_alignment va_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_normal_image(const c_string& name_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_pressed_image(const c_string& name_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_disable_image(const c_string& name_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_nor_fragment(const c_float2& anc_, const c_float2& dim_)
	{
	...
	//--------------------------------------------------------
	void c_radio::set_pre_fragment(const c_float2& anc_, const c_float2& dim_)
	{
		c_skin* sk = c_ui_manager::get_singleton_ptr()->get_skin(m_skinid);
		f32 w = sk->get_skin_tex()->get_width();
		f32 h = sk->get_skin_tex()->get_height();
	...
	//--------------------------------------------------------
	void c_radio::set_dis_fragment(const c_float2& anc_, const c_float2& dim_)
	{
		c_skin* sk = c_ui_manager::get_singleton_ptr()->get_skin(m_skinid);
		f32 w = sk->get_skin_tex()->get_width();
		f32 h = sk->get_skin_tex()->get_height();
	...
	//--------------------------------------------------------
	void c_radio::set_stencil_image(const c_string& img_)
	{
	...
	//--------------------------------------------------------
	void c_radio::_zoom_update(const c_float2& v1_, const c_float2& v2_)
	{	
	...
	void c_radio::_on_touch_finish()
	{
		...
		_sig_state_change.emit(m_state == 1?true:false);
		m_state = (m_state == 1)?0:1;
		c_ui_manager::get_singleton_ptr()->set_dirty(true);
	}
	//--------------------------------------------------------
	void c_radio::_redraw()
	{
		if(!m_visible)
			return;
		c_rect rc;
