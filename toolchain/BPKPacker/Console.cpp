...
static unsigned int hash(const char*  _Str)
{
...
static std::string queryFileMd5(const char* name)
{
	//!test
...
static unsigned char* queryFileData(const char* name, size_t& sz2_, size_t& osz)
{
	FILE* fp = fopen(name, "rb");
	fseek(fp, 0, SEEK_END);
	size_t sz = ftell(fp);
...
void Console::queryFileStream(const char* name, c_stream& mem, size_t& sz2_, size_t& osz)
{
	FILE* fp = fopen(name, "rb");
	fseek(fp, 0, SEEK_END);
	size_t sz = ftell(fp);
