...
void a52_imdct_init (uint32_t mm_accel)
{
    ...
    sum++;
    for (i = 0; i < 256; i++)
	a52_imdct_window[i] = (float)SAMPLE (sqrt (local_imdct_window[i] / sum));

    for (i = 0; i < 3; i++)
	roots16[i] = (float)SAMPLE (cos ((M_PI / 8) * (i + 1)));

    for (i = 0; i < 7; i++)
	roots32[i] = (float)SAMPLE (cos ((M_PI / 16) * (i + 1)));

    for (i = 0; i < 15; i++)
	roots64[i] = (float)SAMPLE (cos ((M_PI / 32) * (i + 1)));

    for (i = 0; i < 31; i++)
	roots128[i] = (float)SAMPLE (cos ((M_PI / 64) * (i + 1)));

    for (i = 0; i < 64; i++) {
	k = fftorder[i] / 2 + 64;
	pre1[i].real = (float)SAMPLE (cos ((M_PI / 256) * (k - 0.25)));
	pre1[i].imag = (float)SAMPLE (sin ((M_PI / 256) * (k - 0.25)));
    }
    ...
    for (i = 0; i < 64; i++) {
	post1[i].real = (float)SAMPLE (cos ((M_PI / 256) * (i + 0.5)));
	post1[i].imag = (float)SAMPLE (sin ((M_PI / 256) * (i + 0.5)));
    }

    for (i = 0; i < 64; i++) {
	k = fftorder[i] / 4;
	pre2[i].real = (float)SAMPLE (cos ((M_PI / 128) * (k - 0.25)));
	pre2[i].imag = (float)SAMPLE (sin ((M_PI / 128) * (k - 0.25)));
    }

    for (i = 0; i < 32; i++) {
	post2[i].real = (float)SAMPLE (cos ((M_PI / 128) * (i + 0.5)));
	post2[i].imag = (float)SAMPLE (sin ((M_PI / 128) * (i + 0.5)));
    }
